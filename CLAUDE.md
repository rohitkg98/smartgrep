...
Dependency kinds emitted: Import, Call (per function/method body; instance calls reduced to the method name), Implements, TypeRef (function/method → project types in its param/return types) and FieldType (type → project types in its field types); TypeRef/FieldType are derived by the index builder, not the parsers. `refs`/`implementing` match by name: bare names match the last path segment (`refs Index` finds `use crate::index::types::Index`), qualified names match as a suffix (`refs User::new`, `refs fmt.Errorf`). Name-based, not type-resolved: `refs new` returns every `new` call.
